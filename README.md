...
Inside Pi, run `/clawchat-activate <invite-code>`. The Extension uses the same
Host Profile and shared Gateway implementation as the executable, but binds
incoming messages to the current interactive Pi session.
...
Tool output defaults to `off`. Change it for the current ClawChat chat with:
